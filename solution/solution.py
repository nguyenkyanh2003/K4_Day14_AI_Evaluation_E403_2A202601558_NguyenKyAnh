...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        Retrieval metrics are diagnostic only and stay out of this average.
        """
...
def _coverage(reference: set[str], covered_by: set[str]) -> float:
    ...
    ``reference`` is the denominator, so it decides what the metric measures:
    the answer for faithfulness, the question for relevance, and the expected
    answer for completeness/recall. An empty reference means there is nothing
    to cover, which scores 1.0 instead of dividing by zero.
    """
    if not reference:
        return 1.0
    return _clamp(len(reference & covered_by) / len(reference))
...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for rank, chunk in enumerate(contexts, start=1):
            coverage = len(expected_tokens & _tokenize(chunk)) / len(expected_tokens)
            if coverage < relevance_threshold:
                continue
            hits += 1
            precision_sum += hits / rank  # Precision@k, counted only at hits

        if hits == 0:
            return 0.0
        return _clamp(precision_sum / hits)
...
class LLMJudge:
    ...
    #: Fallback used when the judge response cannot be parsed into scores.
    DEFAULT_SCORE = 0.5
    ...
    def _parse_scores(raw_response: str) -> dict[str, float]:
        """Extract a criterion → score mapping from a raw judge response.

        The judge is asked for JSON, but real models wrap it in prose or code
        fences, so the first ``{...}`` block is extracted before parsing.
        Anything unparseable yields an empty mapping and the caller falls back
        to DEFAULT_SCORE.
        """
        if not raw_response:
            return {}
        ...
        if not isinstance(payload, dict):
            return {}
        # A judge may nest the numbers under "scores"; accept both shapes.
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial evaluator of a customer-support answer.\n"
            "Score the ANSWER against every rubric criterion.\n"
            ...
            "between 0 and 1, then one sentence of justification.\n"
        )
        ...
        for name in rubric:
            ...
            if value is None:
                ...
                continue
            # Accept a 1-5 rubric score and normalise it onto the 0-1 contract.
            scores[name] = _clamp((value - 1.0) / 4.0 if value > 1.0 else value)

        return {"scores": scores, "reasoning": raw_response or ""}
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        per_response = [mean_of(entry) for entry in scores_batch or []]
        if not per_response:
            return {
        ...
        # Positional bias needs something to compare against: the response shown
        # first is only suspect if it outscores the rest of the batch.
        positional = len(per_response) > 1 and per_response[0] > (
            sum(per_response[1:]) / len(per_response[1:])
        )
...
def _mean(values: Iterable[float]) -> float:
    """Average of a metric across results; 0.0 for an empty benchmark."""
    collected = list(values)
    return sum(collected) / len(collected) if collected else 0.0


def _optional_mean(values: Iterable[float | None]) -> float | None:
    """Average of the recorded scores only; None when nothing was recorded."""
...
class BenchmarkRunner:
    ...
    #: A metric average dropping more than this vs baseline counts as a regression.
    ...
    def run(
        ...
        for pair in qa_pairs:
            ...
            result = evaluator.run_full_eval(
                ...
                # No retrieval trace means the two retrieval metrics stay None.
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        failure_types: dict[str, int] = dict(
            Counter(
                result.failure_type
...
class FailureAnalyzer:
    ...
    def categorize_failures(
        ...
        return dict(
            Counter(
                failure.failure_type or "unclassified" for failure in failures
            )
        )
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        # A tie means no single stage explains the failure on its own.
        if len(weakest) > 1:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures):
            ...
            if suggestions:
                ...
                fix = "Investigate — no suggestion generated"
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        by_type = {
            "hallucination": (
                "Add a grounding check that drops claims not supported by the "
                "retrieved chunks before the answer is returned"
            ),
            "irrelevant": (
                "Tighten the system prompt to restate the user question and "
                "answer it directly instead of summarising the context"
            ),
            "incomplete": (
                "Raise top-k and chunk size so multi-condition policies "
                "(dates, amounts, exceptions) arrive intact in one context"
            ),
            "off_topic": (
                "Add intent routing so each question is matched to the right "
                "policy document before generation"
            ),
            "refusal": (
            ...
            "unclassified": (
                "Review the failing traces manually and extend the failure "
                "taxonomy to cover them"
            ),
        }

        counts = self.categorize_failures(failures)
        # Cluster first, fix the biggest cluster first: one root-cause fix can
        # clear several failures at once.
        suggestions = [
            by_type.get(failure_type, by_type["unclassified"])
        ...
        fallbacks = [
            "Add few-shot examples of complete, evidence-cited answers to the "
            "generation prompt",
            "Rerank retrieved chunks so the most relevant evidence leads the "
            "context window",
            "Grow the golden dataset with variants of the failing cases and "
            "re-run the benchmark as a regression gate",
        ]
